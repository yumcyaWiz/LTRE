...
int main() {
  ...
  const auto floor_tex = std::make_shared<UniformTexture<Vec3>>(Vec3(0.9f));
  const auto floor_material = std::make_shared<Diffuse>(floor_tex, 0.2f);
  scene.addPrimitive(Primitive(floor_shape, floor_material));

  const auto light_shape =
      std::make_shared<Plane>(Vec3(-5, 20, -5), Vec3(10, 0, 0), Vec3(0, 0, 10));
  const auto light_ke = std::make_shared<UniformTexture<Vec3>>(Vec3(0.9f));
  const auto light_le = std::make_shared<UniformTexture<Vec3>>(Vec3(15.0f));
  const auto light = std::make_shared<AreaLight>(light_le, light_shape);
  ...
  for (int j = 0; j < 9; ++j) {
    for (int i = 0; i < n_spheres; ++i) {
      ...
      const float z = 9.0f * 1.2f * radius * v;
      const auto shape = std::make_shared<Sphere>(Vec3(x, 0, z), radius);
  ...
  const auto camera = std::make_shared<ThinLensCamera>(
      Vec3(0, 15, -1), normalize(-Vec3(0, 15, -1)), Vec2(0.025, 0.025),
      PI / 2.0f);
